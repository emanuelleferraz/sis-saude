...
@Repository
public interface DashboardEpidemiologicaRepository extends JpaRepository<ConsultaEntity, Long> {
    ...
    @Query("""
        SELECT new br.edu.ufop.tcc.sis_api.model.dto.dashboards.indicadores.CardResumoDoencaDTO(
            ...
            COUNT(DISTINCT p.id),
            SUM(CASE 
                WHEN c.dataConsulta >= :dataLimite
                THEN 1 ELSE 0 END)
        )
        ...
        JOIN p.endereco e
        JOIN e.bairro b
        WHERE c.doenca.tipo = 'TRANSMISSIVEL'
    """)
    CardResumoDoencaDTO buscarResumo(LocalDateTime dataLimite);
    ...
    @Query("""
        ...
        FROM ConsultaEntity c
        WHERE c.doenca.tipo = 'TRANSMISSIVEL'
        GROUP BY EXTRACT(YEAR FROM c.dataConsulta),
                 EXTRACT(QUARTER FROM c.dataConsulta)
        ORDER BY EXTRACT(YEAR FROM c.dataConsulta),
                 EXTRACT(QUARTER FROM c.dataConsulta)
    """)
