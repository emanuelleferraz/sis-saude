...
import {
  DashboardVacinaDTO,
  CoberturaVacinalBairroDTO,
  DistribuicaoVacinaDTO,
  EvolucaoDosesTrimestreDTO,
  AplicacaoPorUnidadeDTO,
  CardResumoVacinaDTO,
} from "@/types/dashboardVacina";
...
export default function DashboardVacinacao({
  onBack,
  onNavigate,
}: DashboardVacinacaoProps) {
  const [menuOpen, setMenuOpen] = useState(false);
  const [resumo, setResumo] = useState<CardResumoVacinaDTO | null>(null);
  const [coberturaBairros, setCoberturaBairros] = useState<
    CoberturaVacinalBairroDTO[]
  >([]);
  const [distribuicaoDoses, setDistribuicaoDoses] = useState<
    DistribuicaoVacinaDTO[]
  >([]);
  const [evolucaoTrimestral, setEvolucaoTrimestral] = useState<
    EvolucaoDosesTrimestreDTO[]
  >([]);
  const [aplicacoesUnidade, setAplicacoesUnidade] = useState<
    AplicacaoPorUnidadeDTO[]
  >([]);
  ...
  const vacinaMaisAplicada =
    distribuicaoDoses.length > 0
  ...
  const tabelaUnidades = aplicacoesUnidade.reduce(
          ...
          <main className="flex-1 p-8">
            ...
            <div className="mb-6">
              ...
              <p className="text-gray-600">
                Monitoramento da cobertura vacinal por bairro e unidade PSF
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              ...
              <Card className="border-l-4 border-l-green-600">
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {resumo?.vacinaMaisAplicada ?? "..."}
                  </div>
                  <p className="text-xs text-gray-600 mt-1">
                    {vacinaMaisAplicada?.totalAplicacoes ?? "..."} doses
                    aplicadas
                  </p>
                </CardContent>
              </Card>
              ...
              <Card className="border-l-4 border-l-orange-600">
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {resumo?.bairroMenorCobertura ?? "..."}
                  </div>
                  <p className="text-xs text-gray-600 mt-1">
                    {coberturaBairros.find(
                      (b) => b.bairro === resumo?.bairroMenorCobertura,
                    )?.totalAplicacoes ?? "..."}{" "}
                    doses aplicadas
                  </p>
                </CardContent>
            ...
            <Card className="mb-8">
              <CardHeader>
                ...
                <CardDescription>
                  Doses aplicadas em cada bairro
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={coberturaBairros}>
                    ...
                    <Legend />
                    <Bar
                      dataKey="totalAplicacoes"
                      name="Total Aplicações"
                      fill="#3b82f6"
                    />
                  </BarChart>
                </ResponsiveContainer>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
              <Card>
                <CardHeader>
                  <CardTitle>
                    Distribuição de Doses Aplicadas por Vacina
                  </CardTitle>
                  <CardDescription>
                    Proporção de doses por tipo de vacina
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                      <Pie
                        data={distribuicaoDoses.map((d) => ({
                          name: d.nomeVacina,
                          doses: d.totalAplicacoes,
                          fill: "#3b82f6",
                        }))}
                        cx="50%"
                        cy="50%"
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Total de doses aplicadas por trimestre
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart
                      data={evolucaoTrimestral.map((e) => ({
                        trimestre: `T${e.trimestre}/${e.ano}`,
                        doses: e.totalDoses,
                      }))}
                    >
                      <CartesianGrid strokeDasharray="3 3" />
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Detalhamento de doses aplicadas por unidade e tipo de vacina
                </CardDescription>
              </CardHeader>
              <CardContent className="overflow-auto max-h-100">
                <Table className="min-w-200">
                  ...
                  <TableBody>
                    {Object.entries(tabelaUnidades).map(
                      ([unidade, vacinas]) => (
