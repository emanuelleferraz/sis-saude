...
export default function EnderecosPage({ onBack }: EnderecosPageProps) {
  ...
  useEffect(() => {
    if (selectedEndereco) {
      ...
      const bairro = bairros.find(
        (b) => b.nome === selectedEndereco.bairroNome,
      );
  ...
  const handleUpdateEndereco = async () => {
    if (!selectedEndereco || !rua || !numero || !cep || !cidade || !idBairro)
      return;
  ...
  const handleDeleteEndereco = async () => {
    ...
    try {
      ...
      if (
        ...
        err.message.includes("violação de chave estrangeira")
      ) {
        alert(
          "Não é possível deletar este endereço porque ele está em uso por uma unidade.",
        );
      } else {
